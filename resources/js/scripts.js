    ...
    if (keyword.length >= 3) {
      ...
      if (empty) {
        ...
        readTextFile(game_config.domain_root + "games.json", function (text) {
          ...
          if (
            $(".search-results").find(".search__result_container").length > 0
          ) {
            $(".search-results").find(".search__result_container").remove();
          }
          let str_html = `<div class="search__result_container absolute z-50 left-0 right-0 mt-6 rounded-3xl shadow-2xl shadow-[#27391C]/10 dark:shadow-[#27391C]/5 max-h-[60vh] overflow-hidden dark:bg-gray-900/98 backdrop-blur-sm transition-all duration-300 animate-in fade-in slide-in-from-top-2"> <div class="overflow-y-auto max-h-[60vh] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-[#27391C]/5 dark:[&::-webkit-scrollbar-track]:bg-[#27391C]/10 [&::-webkit-scrollbar-thumb]:bg-[#27391C]/30 dark:[&::-webkit-scrollbar-thumb]:bg-[#27391C]/20 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:rounded-full"> <div class="divide-y divide-[#27391C]/10 dark:divide-[#27391C]/5">`;
          if (games.length > 0) {
...
function menu_sidebar_active() {
  ...
  $(".side-nav ul li a").each(function () {
    ...
    if (menu_link == current_url) {
      ...
      $(this).removeClass(
        "text-gray-600 dark:text-gray-300 hover:bg-gradient-to-r hover:from-[#27391C]/5 hover:to-[#4ade80]/5 dark:hover:from-[#27391C]/10 dark:hover:to-[#4ade80]/10",
      );
      $(this).addClass("bg-gradient-to-r from-[#27391C] to-[#4ade80]");
...
$(document).on("click", function (e) {
  if (
    !$(e.target).closest("#contact-popup").length &&
    !$(e.target).closest(".report_bug").length
  ) {
    report_close();
  }
});
...
function notify(message, time, direction = "bt") {
  ...
  html +=
    '<div class="notify-toast" style="position: fixed; bottom: 20px; right: -300px; z-index: 1000;">';
  ...
  if (direction === "rt") {
    ...
    // Default behavior (bottom to top assumed for "bt")
    $notification
...
function notifical_show(id, name, image, slug, favorited, e) {
  let str = "";
  str +=
    '<span class="svg-icon svg-icon--heart btn__icon" aria-hidden="true"> <svg class="svg-icon__link"> <use xlink:href="#icon-heart"></use> </svg> </span>';
  str += `
  ...
  let html = "";
  html +=
    '<div class="notification-success"> <div class="toastt"> <div class="content_notification"> <div class="icon"><img width="50" height="50" src="' +
    image +
...
function save_wishlish_cookies(_id, _slug, _image, _name, _url) {
  ...
  if (
    !!readFromLocalStorage("favorite_game") &&
    _slug !== "" &&
    _image !== "" &&
    _id !== "" &&
    _name != ""
    ...
    if (favorites.length > 0) {
      ...
      for (var i = $leng - 1; i >= 0; i--) {
        ...
        str_wishlist +=
          '<a href="' +
          value.url +
          '" class="thumb-card relative group relative block h-full overflow-hidden rounded-xl bg-white/95 dark:bg-neutral-900/95 backdrop-blur-md shadow-lg hover:shadow-2xl transform transition-all duration-300 hover:-translate-y-1.5 will-change-transform"><div class="w-full overflow-hidden"><img src="' +
          value.image +
          '" alt="' +
          value.name +
          '" sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 20vw" class="object-cover" loading="lazy" style=""></div><div class="px-2 py-2 title-game"><span class="text-sm md:text-base font-medium text-[#27391C] group-hover:text-[#00a35a] transition-colors duration-300 line-clamp-2">' +
          value.name +
          "</span></div></a>";

        if (
          value.slug === game_config.current_slug &&
      ...
      if ($(".favorites_btn").hasClass("favorited")) {
        str +=
          '<div class="g-footer__button-title"> <span class="g-footer__button-title_text g-footer__button-title_first"></span> <span class="g-footer__button-title_text g-footer__button-title_last">Remove</span> </div>';
      } else {
        str +=
          '<div class="g-footer__button-title"> <span class="g-footer__button-title_text g-footer__button-title_first"></span> <span class="g-footer__button-title_text g-footer__button-title_last">Add</span> </div>';
      }
